...
export function Stats() {
  ...
  return (
    <section id={config.id} className="py-12 bg-white relative z-20 -mt-8 mx-4 md:mx-auto md:max-w-5xl rounded-3xl shadow-xl border border-gray-100">
      <div className="container px-4">
        ...
        >
          {items.map((item: any, index: number) => {
            ...
            const IconName = item.icon ? item.icon.split('-').map((word: string) => word.charAt(0).toUpperCase() + word.slice(1)).join('') : null;
